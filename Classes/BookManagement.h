...
class BookManagement {
    list<Book> BookMgm;        //图书列表
    int BookNums;              //图书总数
public:
    BookManagement(){              //图书管理构造函数
        InputBooksFromFile();
        BookNums=static_cast<int>(BookMgm.size());
    }
    int get_BookNums(){            //返回书数目
        return BookNums;
    }
    ...
    void DeletBook(Book &book){    //按对象删除图书
        BookMgm.remove(book);
        BookNums--;
    }
    void DeletBook(long long ISBN){//按唯一标识ISDN删除书籍
        Book temp;
        FindBook(ISBN,temp);
        BookMgm.remove(temp);
        BookNums --;
    }
    void ModifyBook(Book &oldBook, Book &newBook){//修改图书
        DeletBook(oldBook);
        AddBook(newBook);
    }
    bool FindBook(long long ISBN,Book& ret){//按ISDN查找图书
        //list迭代器遍历
    ...
    Book& get_Book_ByIndex(int i){  //按索引查找图书
    ...
    bool OutputBooksToFile(){       //保存文件操作（类似于管理员类）
        QFile file("E:\\Demo\\LibraryManagementByQt-master\\SourceCode\\Classes\\Data\\book_data.txt");
    ...
    bool InputBooksFromFile(){                  //从文件输入数据（类似于管理类）
        BookNums=0;
        BookMgm.clear();
        QFile file("E:\\Demo\\LibraryManagementByQt-master\\SourceCode\\Classes\\Data\\book_data.txt");
